...
#ifndef TILE_HEIGHT
#error "Please add a TILE_HEIGHT definition in the tileset!"
#endif
...
** the image may be fixed). The "rjmp" is not added where at this stage a
** "jmp" might fit (maximal size of head is 4 instruction words).
**
  ...
  for (ii = 0U; ii < 256U; ii ++){
   ...
   while (ss < 4U){
    if ((ins_prep[jj][ii][kk] & INS_TAIL) == 0U){
     ins_gen[posx + (ii * 4U) + ss] = ins_prep[jj][ii][kk];
     ss ++;
     if (ins_prep[jj][ii][kk] == INS_RJMP0){
      jump_addr[posx + (ii * 4U) + ss - 1U] = MAX_INS;
  ...
  ** jump tables). The ins_gen table contains INS_UNF in open locations. */
  for (ii = 0U; ii < 1024U; ii ++){
   if ((row_rank[ii & 0xFFU] & 3U) == (ii >> 8)){
    ...
    pjmp = (posx + ((ii & 0xFFU) * 4U)) + (nn - 1U); /* Position of jump */
